...
**Closing note (read before starting Project 1):** this repo has starter applications at [`Backend_Developer/03_Interview_AnyYear/03_Projects/`](../../Backend_Developer/03_Interview_AnyYear/03_Projects/), but as of the last repo-wide audit only **`08_FastAPI_OpenAI_RAG_Backend_starter`** is actually real — working routers, retrieval logic, DB session handling. `05_Django_Banking_Fintech_starter` and `06_Django_HR_Payroll_starter` are still TODO scaffolds (a `manage.py` whose own comment says `django-admin startproject` hasn't been run yet) — don't pick either expecting an app to containerize; you'd be building the Django app itself first, which is a different, larger task than this DevOps track. Use `08_FastAPI_OpenAI_RAG_Backend_starter` as the app you deploy across all 10 projects instead of building a throwaway "hello world" — it already has real Postgres+Redis dependencies, giving you actual production concerns to solve.

**Real progress exists:** [`capstone_rag_backend/`](capstone_rag_backend/) has verified, working artifacts for Projects 1, 2, 4, and 5 against this exact starter — a production Dockerfile (built and tested), a CI workflow template, modularized Terraform, and Kubernetes manifests. Read its README for what's actually been run vs. what's written-but-unapplied before continuing from Project 3 or 6 onward.
