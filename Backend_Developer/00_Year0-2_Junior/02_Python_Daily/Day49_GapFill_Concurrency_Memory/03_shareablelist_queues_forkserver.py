...
PREREQUISITE: Day48/01_multiprocessing_shared_state.py PEHLE padho — wahan
...
WHY THIS MATTERS:
- Day48 me dekha: SharedMemory = raw bytes. Par real code me tumhe "list of
  mixed values" share karni hoti hai — uske liye ShareableList hai (raw buffer
  ke upar list-like API, ZERO pickle per access).
- Day48 me dekha: close()/unlink() bhoolna = resource leak warning. Production
...
BIG PICTURE MAP:
    SharedMemory (Day48)      → raw bytes buffer (tum layout manage karo)
...
spawn/forkserver child me module re-import + target pickle hota hai (Day48 Q6).
...
# Day48 me raw bytes the (buf[i] = 42) — yahan sl[0] = "hello" jaisa kaam karta hai.
#
...
def shareablelist_child(shm_name):
    ...
    sl.shm.close()             # apna view band (unlink NAHI — creator karega, Day48 rule)
...
def demo_shareablelist():
    ...
    try:
        ...
        sl.shm.unlink()     # physical block free — Day48 wala resource leak rule
    ...
    # round-trip to manager server process (Day48 Section 2). Farak DEKHO:
    N_OPS = 3000
...
#
# Day48 ka PAIN yaad karo: har SharedMemory pe close() HAR process me + unlink()
...
def demo_shared_memory_manager():
    ...
    print("\nManual (Day48)            vs  SharedMemoryManager (aaj):")
